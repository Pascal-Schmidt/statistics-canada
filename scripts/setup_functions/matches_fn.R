...
matches <- function(df) {
  ...
  df %>%
    ...
    dplyr::mutate(match_IRCC_ER = ifelse(IRCC_ER == "FALSE", "No Match", match_IRCC_PROV)) %>%
    dplyr::select(-c(ar_uid:derived_da2016, address_mun, ER_CSDD:ARUID_CENSUS, IRCC_ARUID:CSDD_CD)) %>%
    dplyr::filter(validity == "valid_during_census") %>%
    dplyr::filter(match_IRCC_ARUID == "Match" | match_IRCC_ARUID == "No Match" | is.na(match_IRCC_ARUID)) -> df

  return(df)

  # CID_during_Census_student <<- df$CID
}
