...
validity <- function(df) {
  ...
  df %>%
    ...
    dplyr::mutate(
      ...
      closest_to_census = as.character(census_date - lubridate::as_date(EFF_DT)) %>%
        as.integer(),
    ...
    dplyr::mutate(validity = ifelse(lubridate::as_date(EFF_DT) < lubridate::ymd("2016-05-10") & lubridate::as_date(VALID_DT) < lubridate::ymd("2016-05-10"),
      "valid_before_census",
      NA
    )) %>%
    dplyr::mutate(validity = ifelse(lubridate::as_date(EFF_DT) > lubridate::ymd("2016-05-10") & lubridate::as_date(VALID_DT) > lubridate::ymd("2016-05-10"),
