...
# read in data into r
pr_emp <- haven::read_sas("//cptsas1/Demographie/Adresses IRCC/5_Programmes/merged_employ_permits_pr.sas7bdat")
...
# remove rows/CIDs that are in pr_emp but not in census/CSDD
pr_emp %>%
  ...
  {
    which(. != {
      dplyr::select(pr_emp, SOURCE_FLAGS:ARUID_CENSUS)
    } %>%
      names() %>%
      length())
  } %>%
...
pr_emp %>%
  ...
  {
    which(. != {
      dplyr::select(pr_emp, EFF_DT, VALID_DT)
    } %>%
      names() %>%
      length())
  } %>%
...
pr_emp %>%
  dplyr::filter(LAND_DT <= lubridate::ymd("2016-05-10")) -> pr_emp

# write csv file
readr::write_csv(pr_emp, "//cptsas1/Demographie/Adresses IRCC/5_Programmes/Data/pr_emp.csv")
